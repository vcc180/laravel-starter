...
@section('content')
<div class="card">
    <div class="card-header"><h1 class="h5 mb-0">Editar perfil</h1></div>
    <div class="card-body">
        <form method="post" action="{{ route('admin.roles.update', $item) }}">
            @csrf @method('PUT')
            <div class="mb-3">
                        ...
                        @foreach($items as $permission)
                            <div class="col-md-4 mb-2">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="permissions[]" id="p{{ $permission->id }}" value="{{ $permission->id }}" {{ in_array($permission->id, $selected) ? 'checked' : '' }}>
